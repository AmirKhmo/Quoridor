    ...
    //when player2 should enter sth , Board's color will be blue
                ...
                if(ascii==80 && a==xx+2 && yy==b && board[a+1][b]!=wall) xx+=4;// ASCII code of up & down & left & right button
                    ...
                    if(wallCount1>0){
                        ...
                        if(wallDirection=='v' || wallDirection=='V'){
                            ...
                            }while(DFS(2*h-1,2*v,walls,k,x,y,a,b,1)!=1 || q!=1);// we use do...while because wall's coordinates should be in the board and should be in the free places
                            h=2*h-1;
                            v=2*v;
                            walls[h][v]=wall;
                            walls[h+1][v]=wall;
                        ...
                        if(wallDirection=='h' || wallDirection=='H'){
                            ...
                            }while(DFS(2*h,2*v-1,walls,k,x,y,a,b,2)!=1 || q!=1);// we use do...while because wall's coordinates should be in the board and should be in the free places
                            h=2*h;
                            v=2*v-1;
                            walls[h][v]=wall;
                            walls[h][v+1]=wall;
        ...
        int computer = makeRandom();
            ...
            do{//computer next move
                ...
                if(move == 1 && x==aa+2 && bb==y) aa+=4;// ASCII code of up & down & left & right button
                else if(move == 1) aa+=2;
